...
public class IntroSceneState extends BasicGameState {
	...
	public void update(GameContainer container, StateBasedGame game, int delta)
		...
		if(this.currentSlide == SLIDE.SLIDE1) {
			if(this.slide1.getX() < 520) {
				this.slide1.setX(this.slide1.getX() + this.slide1.getMoveBy());
			} else { // start slide 2 
			...
			if(this.slide3.getX() > 200) {
				this.slide3.setX(this.slide3.getX() - this.slide3.getMoveBy());
			} else {
				this.currentSlide = SLIDE.SLIDE4;
			}
		} else if (currentSlide == SLIDE.SLIDE4) {
			if(this.slide4.getX() < 650) {
				this.slide4.setX(this.slide4.getX() + this.slide4.getMoveBy());
			} else {
				this.currentSlide = SLIDE.SLIDE5;
			}
		} else if(currentSlide == SLIDE.SLIDE5) {
			if(this.slide5.getX() < 520) {
				this.slide5.setX(this.slide5.getX() + this.slide5.getMoveBy());
			} else {
				//this.currentSlide = SLIDE.SLIDE2; 
			} 	
		}
